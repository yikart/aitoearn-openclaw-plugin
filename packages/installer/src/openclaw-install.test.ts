...
describe("installPluginWithOpenClaw", () => {
  const packageContext: PackageContext = {
    ...
    runtimeInstallSpec: "@aitoearn/openclaw-plugin@1.0.15",
  ...
  it("calls openclaw plugins install for a fresh install", async () => {
    ...
    const readConfig = vi.fn().mockResolvedValue({
      plugins: {
        installs: {
          aitoearn: {
            source: "npm",
            spec: "@aitoearn/openclaw-plugin@1.0.15",
            resolvedSpec: "@aitoearn/openclaw-plugin@1.0.15",
          },
        },
      },
    });
    ...
    expect(runOpenClaw).toHaveBeenCalledWith(
      ["plugins", "install", "@aitoearn/openclaw-plugin@1.0.15"],
      {
